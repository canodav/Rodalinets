...
const NearestStationOption = () => {
  ...
  const formatDistance = (distance: number) => {
    let distanceString = "" 
    if(distance < 1 ) { //print it like meters
  ...
  return (
      <View>
        <Pressable style={{paddingVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 5,}} onPress={getNearestStation}>
            ...
            <Text style={{color: "#00A0D3", fontFamily: "Poppins_Bold"}}>Nearest Station  {nearestStation && 
              <Text style={{color: Colors.text, fontFamily: "Poppins_Bold"}}>{ nearestStation.name + " ≈ " + formatDistance(nearestStation.distance) }</Text>
            } </Text>
