...
export const StationProgress = ({
    ...
    function calculateTrainPositionPercentage(trainCoords: coords, stations) {
        const distance = (coord1, coord2) => {
            return Math.sqrt(
                Math.pow(coord2.lat - coord1.lat, 2) +
                    Math.pow(coord2.lon - coord1.lon, 2)
            );
        };
        const stationDistances = stations.map((station) => ({
            ...station,
            distance: distance(
                { lat: trainCoords[0], lon: trainCoords[1] },
                { lat: station.coords.latitude, lon: station.coords.longitude }
            ),
        }));

        const sortedStations = stationDistances.sort(
        ...
        const closestStationPercentage =
            stations.findIndex(
                (station) => closestStations[0].coords == station.coords
            ) * 25;
    ...
    useEffect(() => {
        if (trainPosition && stations.length > 0) {
            const percentage = calculateTrainPositionPercentage(
                trainPosition,
                stations
            );
            ...
            <View style={styles.container}>
                ...
                {stations.map((station, index) => (
                    <View
                        ...
                        style={[
                            styles.station,
                            station.id === currentStation.id ? styles.currentStation : null,
                            { left: `${index * stationWidth - 1}%` },
                        ]}
                    >
